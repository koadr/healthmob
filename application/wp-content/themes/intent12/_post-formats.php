		...
		<div class="flexslider flex-post fix" id="slider-<?php the_ID(); ?>">
			<ul class="slides">
				<?php foreach($images as $image): ?>
					<li>
						<?php $image640=wp_get_attachment_image_src($image->ID,'post-format'); ?>
						<img src="<?php echo $image640[0]; ?>" alt="<?php echo $image->post_title; ?>">
...
<?php if(has_post_format( 'image' )): // Post Format: Image ?>
	...
	<?php $images=bandit::get_post_images();
		if(!empty($images) && (1==count($images)) && !has_post_thumbnail()) {
			$image640=wp_get_attachment_image_src($images[0]->ID,'post-format');
			$image640['title']=$images[0]->post_excerpt;
		} elseif (has_post_thumbnail()) {
			$img_id = get_post_thumbnail_id();
			$image640['title'] = get_post_field('post_excerpt', $img_id);
		}
	?>
	<div class="entry-format image fix">
		<div class="image-container">
			<?php echo (isset($meta['_image_url'][0])?'<a href="'.$meta['_image_url'][0].'">':''); ?>
				...
				<?php if(isset($image640) && !has_post_thumbnail()) { echo '<img src="'.$image640[0].'" alt="'.$images[0]->post_title.'" >'; } ?>
			<?php echo (isset($meta['_image_url'][0])?'</a>':''); ?>
			<?php if(isset($image640['title']) && $image640['title']): ?>
				<span class="caption-bar"><i><?php echo $image640['title']; ?></i></span>
			<?php endif; ?>
		</div>
	</div>
...
<?php if (has_post_format( 'video' )): // Post Format: Video ?>
	...
	<div class="entry-format video fix">
		<?php 
		if(isset($meta['_video_url'][0])) {
			global $wp_embed;
			$video = $wp_embed->run_shortcode('<div class="video-container">[embed]'.$meta['_video_url'][0].'[/embed]</div>');
			echo $video;
		} elseif(isset($meta['_video_embed_code'][0])) {
